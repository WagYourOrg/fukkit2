...
public abstract class RemovingTargetAnnotationHandler<T extends Annotation> extends RemovingAnnotationHandler<T> {
    ...
    @Override
    public final void transform(T annotation, TransformerManager transformerManager, IClassProvider classProvider, Map<String, IInjectionTarget> injectionTargets, ClassNode transformedClass, ClassNode transformer, FieldNode transformerField) {
        for (String targetCombi : this.targetCombis.apply(annotation)) {
            List<FieldNode> targets = ASMUtils.getFieldsFromCombi(transformedClass, targetCombi);
            if (targets.isEmpty()) throw new MethodNotFoundException(transformedClass, transformer, targetCombi);
            for (FieldNode target : targets) {
                this.transform(annotation, transformerManager, classProvider, injectionTargets, transformedClass, transformer, cloneField(transformerField), target);
            }
        }
    }
     ...
     * @param target             The currently transformed target {@link MethodNode}
     */
    public abstract void transform(final T annotation, final TransformerManager transformerManager, final IClassProvider classProvider, final Map<String, IInjectionTarget> injectionTargets, final ClassNode transformedClass, final ClassNode transformer, final FieldNode transformerMethod, final FieldNode target);
